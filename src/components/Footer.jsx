          ...
          {footerLinks.map((footerLink) => (
            <div key={footerLink.title} className='flex flex-col ss:my-0 min-w-[150px]'>
              ...
              {footerLink.links.map((link, index) => (
                <li key={link.name} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite ease-in duration-200 hover:text-secondary cursor-pointer ${index !== link.length -1
                ? 'mb-4' : 'mb-0'}`}>
                  {link.name}
                </li>
              ))}
